...
class PointMovingOnGeodesic {
    ...
    show() {
        ...
        if (Math.sqrt(this.pt.x*this.pt.x + this.pt.y*this.pt.y) < 1){ 
            this.pt.show();
        }  
        //this.geodesic.show();
        pop();
...
class Point {
    ...
    createCircleDir(pt1){ 
        ...
        let eps = 0.00000001
        if (dist(m1,m2,x1,y1) - radius > eps){ //both points has to be on circle -> swap signs
            m1 = this.x + radius*diry;
            m2 = this.y - radius*dirx;
        }

        //create circle as geodesic
        let c = new Circle(m1*w/2, m2*w/2, radius); //multiplied with w/2 to have right position on canvas
        return c;
    }
}
